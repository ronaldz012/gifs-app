...
@Component({
  ...
  template: `
    <div class="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <div class="relative z-10 w-full max-w-md mx-4 bg-bg-surface rounded-2xl border border-border shadow-xl animate-fade-in"
        ...
        <header class="flex items-center justify-between px-5 pt-5 pb-3">
          ...
          <button type="button" (click)="onClose()" class="btn-icon hover:bg-bg-muted hover:text-text-main shrink-0" aria-label="Cerrar">
        ...
        <div class="px-5 space-y-4">
          <div class="flex flex-col gap-1">
            ...
            <input type="text" [formField]="form.name" placeholder="Ej. Sucursal Central" autofocus
          ...
          <div class="flex flex-col gap-1">
            ...
            <input type="text" [formField]="form.place" placeholder="Ej. Av. Principal #123"
          ...
          <div class="flex flex-col gap-1">
            ...
            <input type="text" [formField]="form.phoneNumber" placeholder="Ej. 76543210"
                   class="w-full px-3 py-2 border border-border rounded-lg text-sm text-text-main placeholder:text-text-soft bg-bg-surface font-medium outline-none transition-all focus:ring-2 focus:ring-accent-ui/20 focus:border-accent-ui" />
          </div>

          @if (error()) {
            <div class="px-4 py-3 bg-feedback-error-bg/10 border border-feedback-error-text/20 rounded-lg text-xs text-feedback-error-text font-medium">
              {{ error() }}
            </div>
          }
        </div>

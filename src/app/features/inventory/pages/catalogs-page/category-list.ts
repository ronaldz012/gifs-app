  ...
  onUpdated(dto: UpdateCategoryDto): void {
    ...
    this.service.updateItem(id, dto).subscribe({
      ...
      error: (err: unknown) => {
        ...
        this.toastService.error(e?.error?.detail || e?.error?.title || e?.message || 'Error al actualizar la categoría.');
      },
    });
  }
  ...
  onStatusConfirm(): void {
    ...
    this.service.updateStatus(id).subscribe({
      ...
      error: (err: unknown) => {
        this.saving.set(false);
        const e = err as { error?: { detail?: string; title?: string }; message?: string };
        this.toastService.error(e?.error?.detail || e?.error?.title || e?.message || 'Error al cambiar el estado de la categoría.');
      },
    });
  }

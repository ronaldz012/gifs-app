  ...
  template: `
    <div class="bg-bg-surface border border-border rounded-lg shadow-lg p-4 w-72">
      <div class="flex justify-between items-center mb-4">
        <h3 class="section-title mb-0">Editar Marca</h3>
      ...
      <div class="space-y-3">
        ...
        <div>
          <label class="field-label block">PREFIJO (no editable)</label>
          <input
            type="text"
            [value]="item().prefix"
            readonly
            class="w-full px-3 py-2 rounded-lg border border-border bg-bg-muted text-text-soft text-sm uppercase font-mono tracking-widest outline-none"
          />
        </div>

        <div>
          <label class="field-label block">
            DESCRIPCIÓN <span class="text-text-soft normal-case font-normal">(opcional)</span>
          </label>
          <textarea
            [formField]="editForm.description"
            placeholder="Descripción breve..."
            rows="2"
            class="w-full px-3 py-2 rounded-lg border border-border bg-bg-muted text-text-main
                   text-sm outline-none transition-all resize-none
                   focus:ring-2 focus:ring-[--focus-ring] focus:border-accent-ui
...
export default class EditBrand implements OnInit {
  item = input.required<Brand>();
  saving = input(false);

  saved = output<UpdateBrandDto>();
  closed = output<void>();
  ...
  ngOnInit(): void {
    this.model.set({ name: this.item().name, description: this.item().description });
  }

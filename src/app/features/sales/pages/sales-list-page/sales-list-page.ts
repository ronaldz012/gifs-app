    ...
    <div class="flex flex-col gap-4 w-full">
      ...
      @if (loading()) {
        ...
        >
          ...
          <div class="text-center px-6">
            ...
            <p class="text-xs max-w-xs mt-1">
              No se encontraron ventas{{ hasDateFilter() ? ' con los filtros seleccionados' : '' }}.
            </p>
          </div>
          @if (hasDateFilter()) {
            <button
        ...
        <div class="bg-bg-surface rounded-xl border border-border shadow-xs overflow-hidden">
          <div
            class="hidden lg:grid lg:grid-cols-7 px-4 py-3 bg-bg-muted border-b border-border text-[10px] font-bold uppercase tracking-wider text-text-soft"
          >
            <span>Fecha</span>
          ...
          <ul class="flex flex-col divide-y divide-border">
            @for (sale of sales(); track sale.id) {
              <li
                class="row-enter bg-bg-surface relative overflow-hidden border-b border-border transition-all duration-200 hover:shadow-md"
              >
                <div
                  class="absolute bottom-0 left-0 top-0 w-[4px] bg-accent-ui opacity-0 transition-opacity duration-150 group-hover:opacity-100 lg:block hidden"
                ></div>
                ...
                <div class="flex items-center gap-4 px-4 py-3.5 lg:hidden">
                  <div class="flex flex-col min-w-0 flex-1">
                    ...
                    <p class="mt-1 font-inter text-xs text-text-muted">
                      <span class="font-bold text-accent-ui">{{
                        sale.totalAmount | currency: 'BOB' : 'symbol' : '1.2-2'
                      }}</span>
                      <span
                        class="font-bold text-accent-ui bg-accent-ui/10 px-1.5 py-0.5 rounded text-[11px]"
                        >{{ sale.itemCount }} art.</span
                      >
                ...
                <!-- DESKTOP -->
                <div
                  class="group hidden lg:grid lg:grid-cols-7 items-center px-4 py-3 transition-colors duration-150 hover:bg-bg-muted"
                >
                  <span class="text-[13px] font-medium text-text-main">{{
                    sale.createdAt | smartDate
                  }}</span>
                  ...
                  <span>
                    @if (sale.paymentMethod === 0) {
                      <span
                        class="text-[11px] font-medium text-text-muted bg-bg-muted px-2 py-0.5 rounded-md"
                        >Efectivo</span
                      >
                    } @else {
                      <span
                        class="text-[11px] font-medium text-feedback-info-text bg-feedback-info-bg/10 px-2 py-0.5 rounded-md"
                        >Pago Móvil</span
                      >
                    }
                  </span>
                  <span>
                    @if (sale.documentType === 'Invoice') {
                      <span
                        class="text-[11px] font-medium text-feedback-success-text bg-feedback-success-bg/10 px-2 py-0.5 rounded-md"
                        >Factura</span
                      >
                    } @else if (sale.documentType === 'PendingInvoice') {
                      <span
                        class="text-[11px] font-medium text-feedback-warning-text bg-feedback-warning-bg/15 px-2 py-0.5 rounded-md"
                        >Pendiente</span
                      >
                    } @else {
                      <span
                        class="text-[11px] font-medium text-text-soft bg-bg-muted px-2 py-0.5 rounded-md"
                        >Boleta</span
                      >
                    }
                  </span>
                  <span
                    class="text-center text-xs font-bold font-mono text-accent-ui bg-accent-ui/10 px-2 py-0.5 rounded-md w-fit mx-auto"
                    >{{ sale.itemCount }}</span
                  >
                  <span class="text-xs text-text-soft font-mono truncate">{{
                    sale.invoiceNumber ?? '—'
                  }}</span>
